...
<div class="col-md-6 offset-3">
    <br><h3 align="center">Customer Registration</h3><br>
    ...
    <?php
      if(isset($_POST["submit"])){
        ...
        // error handling
        if(empty($user_name) OR empty($address) OR empty($email) OR empty($password) OR empty($repeat_password)){
        ...
        if(count($errors) > 0){
            ...
            // require_once "database.php";
            $sql = "INSERT INTO customer(customer_name, address, email, password) VALUES(?,?,?,?)";
            $stmt = mysqli_stmt_init($conn);
            $prepareStmt = mysqli_stmt_prepare($stmt, $sql);
            
            // to avoid sql injections
            if($prepareStmt){
                mysqli_stmt_bind_param($stmt, "ssss", $user_name, $address, $email, $passwordHash);
